  ...
  <div v-loading="tableLoading" element-loading-text="加载中...">
    <el-form :inline="true" :model="filter" style="display: flex; justify-content: space-between">
      <div>
        ...
        <el-form-item label="页面位置" prop="title">
          <el-select v-model="filter.title" placeholder="请选择" style="width: 160px" clearable>
            <el-option
    ...
    <el-table :data="tableData" border highlight-current-row style="width: 100%">
      ...
      <el-table-column prop="title" label="页面位置" minWidth="200"></el-table-column>
...
<script>
  ...
  import { BannerTitleLabel } from '@/constant/enums';
  export default {
    name: 'BannerIndex',
    mixins: [options],
    created() {
    ...
    methods: {
      ...
      search() {
        ...
        this.getBannerListWithPage();
      },
      async change(current) {
        this.pagination.currentPage = current;
        await this.getBannerListWithPage(current);
      },
      edit(row) {
        this.$router.push({ path: `/config/banner/edit/${row.id}` });
      },
      async getBannerListWithPage(current = 1) {
        const currentPage = current ? current : this.pagination.currentPage;
        const res = await configurationV1Api.getBannerList({
        ...
        this.tableData = [];
        HandleIndexSession.setSession(this, 'banner');
        items.forEach((e) => {
          this.tableData.push({
            id: e.id,
            title: BannerTitleLabel[e.title] ? BannerTitleLabel[e.title] : e.title,
      ...
      async initList() {
        ...
        if (
          HandleIndexSession.exitSession(this, 'banner') &&
          this.$cookie.get('is_edit_back') == 1
        ) {
          HandleIndexSession.getSession(this, 'banner');
        }
        await this.getBannerListWithPage();
