<template>
  <div>
    <el-form ref="form" :model="detailForm" :rules="rules" label-width="150px" style="width: 500px">
      ...
      <el-form-item label="排序" prop="position" style="width: 500px">
        <el-input v-model.number="detailForm.position" class="input-style"></el-input>
      </el-form-item>
      ...
      <el-form-item style="width: 500px">
        <div style="display: flex;">
          ...
          <el-button type="primary" @click="create('form')" v-show="isNew">新建</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async initData(id) {
        this.setLoading();
        const tmp = await configurationApi.getHomepageItem(id);
        const res = tmp.data;
      ...
      async update(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            const banner = JSON.parse(JSON.stringify(this.detailForm));
            configurationApi
              .updateHomepage(this.detailForm.id, banner, this.originData)
              .then(({ errorCode, message }) => {
                if (errorCode === 0) {
                  Message.success({
                    message: '更新成功！',
